...
- How to use async functions in the useEffect hook
- How to handle errors and use a 'status' state to render things properly and clearly
- How to bundle associated state by using an object with those states together to reduce rerenders
- About the existence of React Error Boundaries and how they can prevent an application from failing in front of the user
- About the existence of the react-error-boundary package and how to use it to reduce code writing
- How to effectively reset an error boundary to allow a restart
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=ryanclott%40outlook.com).
