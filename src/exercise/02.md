...
### New Things!
- I learned:
  - About using localStorage to save persist variables though the browser's storage
  - How to reduce the number of times expensive functions are called in state initialization by use of 'lazy state initialization' which is done by providing a function to the useState hook.
  - About the benefit of using useEffect hook's dependency array
  - How to create my own custom hook
  - About considerations when saving things to localStorage as far as serialization goes
  - how useRef can help keep track of changing vars without triggering additional renders
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=ryanclott%40outlook.com).
